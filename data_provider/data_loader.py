...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # Feature selection
        if self.features == 'M' or self.features == 'MS':
            # Use all features (OHLCV)
            cols_data = df_raw.columns[1:]  # All columns except timestamp
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            # Use only target feature (default: close price)
            df_data = df_raw[[self.target]]
        ...
        df_stamp['timestamp'] = pd.to_datetime(df_stamp['timestamp'])  # Fixed: using 'timestamp' instead of 'date'
        if self.timeenc == 0:
