...
def make_prediction(model, data_x, data_x_mark, pred_len, args, device='cuda'):
    """Make prediction using the loaded model"""
    with torch.no_grad():
        data_x = data_x.to(device)
        data_x_mark = data_x_mark.to(device)
        
        # Create decoder input (zeros for prediction length)
        dec_inp = torch.zeros((1, pred_len, data_x.shape[-1])).float().to(device)
        dec_inp = torch.cat([data_x[:, -args.label_len:, :], dec_inp], dim=1)
        
        # Create decoder time features
        future_timestamps = pd.date_range(
            start=pd.Timestamp.now(),
            periods=args.label_len + pred_len,
            freq=args.freq
        )
        ...
        for t in future_timestamps:
            ...
            future_time_features.append(time_feat)
        dec_mark = torch.FloatTensor(future_time_features).unsqueeze(0).to(device)
        
        # Make prediction
        outputs = model(data_x, data_x_mark, dec_inp, dec_mark)
        
        # Get only the predicted values
        predictions = outputs[:, -pred_len:, :]
...
def main():
    ...
    # Get last seq_len datapoints
    recent_data = df.iloc[-args.seq_len:].copy()
    recent_timestamps = recent_data['timestamp'].tolist()
    ...
    data_x, data_x_mark = prepare_data(
        recent_data.iloc[:, 1:], args, scaler, timestamps=recent_timestamps
    )
    ...
    future_timestamps = [last_timestamp + freq_offset * (i + 1) for i in range(args.label_len + args.pred_len)]
    ...
    # Create prediction timestamps (as UNIX time)
    pred_timestamps = [int((last_timestamp + freq_offset * (i + 1)).timestamp()) for i in range(args.pred_len)]
    
    # Create prediction DataFrame
    pred_df = pd.DataFrame(predictions, index=pred_timestamps, 
    ...
    # Print timestamp and close in CSV format for easy copy-paste
    print("\ntimestamp,close")
    for ts, row in pred_df.iterrows():
        print(f"{ts},{row['close']}")
